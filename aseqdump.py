...
class AseqThread(threading.Thread):
    def run(arg):
        ...
        while running:
            ...
            with data_lock:
                if data and not notes_on and timestamp - (data[-1][0] + data[-1][3]) > 1.5:
                    data2 = []
                    ...
                    rintervals = [d for d in data2 if d.interval_one_hand != 0 and d.hand == 'r']
                    lintervals = [d for d in data2 if d.interval_one_hand != 0 and d.hand == 'l']
                        ...
                        for i, d in enumerate(data2):
                            if not rintervals and not lintervals:
                                if abs(d.interval - mu_int) > pstd_int * 1.5:
                                    print("{}: {}: interval {:.4f} ({:.2f}%)".format(i, d.note, d.interval - mu_dur, (d.interval - mu_int) * 100 / mu_int))
                            if abs(d.velocity - mu_vel) > pstd_vel * 2:
                                print("{}: {}: velocity {:.4f} ({:.2f}%)".format(i, d.note, d.velocity - mu_vel, (d.velocity - mu_vel) * 100 / mu_vel))
                            if abs(d.duration - mu_dur) > pstd_dur * 2:
                                print("{}: {}: duration {:.4f} ({:.2f}%)".format(i, d.note, d.duration - mu_dur, (d.duration - mu_dur) * 100 / mu_dur))

                        if rintervals:
                            d = [i.interval_one_hand for i in rintervals]
                            mu = statistics.mean(d)
                            pstd = statistics.pstdev(d, mu)
                            print("interval (right): mean = {:.4f}, pstdev = {:.4f} ({:.2f}%)".format(mu, pstd, pstd * 100 / mu))

                        if lintervals:
                            d = [i.interval_one_hand for i in lintervals]
                            mu = statistics.mean(d)
                            pstd = statistics.pstdev(d, mu)
                            print("interval (left): mean = {:.4f}, pstdev = {:.4f} ({:.2f}%)".format(mu, pstd, pstd * 100 / mu))

                        print("interval: mean = {:.4f}, pstdev = {:.4f} ({:.2f}%)".format(mu_int, pstd_int, pstd_int * 100 / mu_int))
                        print("velocity: mean = {:.4f}, pstdev = {:.4f} ({:.2f}%)".format(mu_vel, pstd_vel, pstd_vel * 100 / mu_vel))
                        print("duration: mean = {:.4f}, pstdev = {:.4f} ({:.2f}%)".format(mu_dur, pstd_dur, pstd_dur * 100 / mu_dur))
                ...
                if event[0] == 6:
                    ...
                    data.append((timestamp, note_to_str(note), velocity, 0, note_to_color(note)))
                    data_updated = True
                elif event[0] == 7:
                    note = event[7][1]
                    try:
                    ...
                    for k, v in notes_on.items():
                        ...
                        data[v] = (data[v][0], data[v][1], data[v][2], duration, data[v][4])
                        data_updated = True
